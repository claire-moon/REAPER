...
//------------------------------------------------------------------------------------------------------------------------------------------
// An 'angle_t' value that supports being interpolated (according to the world/mobj tic rate) for uncapped framerates
//------------------------------------------------------------------------------------------------------------------------------------------
struct InterpAngle {
    angle_t     value;          // The current value of this field
    angle_t     oldValue;       // The previous value of this field, prior to the current value being set
    int32_t     oldGameTic;     // The game tick in which the old value was last assigned, this is the tick that should be interpolated

    // Assign a new value to the interpolated field and remember the replaced value if from a previous frame
    angle_t operator = (const angle_t& newValue) noexcept;

    inline angle_t operator += (const angle_t& amt) noexcept { return operator = (value + amt); }
    inline angle_t operator -= (const angle_t& amt) noexcept { return operator = (value - amt); }

    // Returns the un-interpolated value of the field.
    // Most of the time this is what is needed (for game logic).
    inline operator angle_t () const noexcept {
        return value;
    }
    ...
    // Set the value and snap to it immediately
    inline void snapToValue(const angle_t newValue) noexcept {
        value = newValue;
        oldValue = newValue;
        oldGameTic = 0;
    }
    ...
    // May modify the old value to speed up future render value queries.
    // The interpolation is done using the world/mobj frame rate of 15 Hz for NTSC.
    angle_t renderValue() noexcept;
    angle_t renderValue(const bool bInterpolate) noexcept;
};
